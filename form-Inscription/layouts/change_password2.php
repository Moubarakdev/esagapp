    ...
    if(!empty($_POST['current_password']) && !empty($_POST['new_password']) && !empty($_POST['new_password_retype'])){
        ...
        if(password_verify($current_password, $data_password['password']))
        {
            if($new_password == $new_password_retype){
                ...
                header('Location: /esagapp/form-inscription/recuperation.php?login_err="success"&err="success_password"');
                die();
            }
        }
        else{
            header('Location: /esagapp/form-inscription/recuperation.php?login_err="success"&err=current_password');
            die();
        }
    }
